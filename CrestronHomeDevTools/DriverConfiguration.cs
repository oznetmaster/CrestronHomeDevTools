...
public static class DriverConfiguration
	...
	public sealed class Inputs
		{
		internal Inputs (IReadOnlyDictionary<string, string>? values, Step[]? steps) { Values = values; Steps = steps; }
		internal IReadOnlyDictionary<string, string>? Values { get; }
		internal Step[]? Steps { get; }
		}

	public static Inputs ReadInputs (string path)
		{
	...
	// Hold the processor operation lease across installation and this call. Mutations are never retried.
	public static async Task<DriverConfigurationResult> ConfigureAsync (ConfigurationClient client, DriverInstanceReady target,
		Inputs inputs, TimeSpan timeout, CancellationToken cancellationToken = default)
		{
		ArgumentNullException.ThrowIfNull (client);
		ArgumentNullException.ThrowIfNull (target);
		ArgumentNullException.ThrowIfNull (inputs);
		if (target.DeviceId <= 0 || string.IsNullOrWhiteSpace (target.Model) || !Version.TryParse (target.Version, out _))
			throw new ArgumentException ("Configuration requires a positive device ID, expected model and version.");
		...
		try
			{
			bool changed = await ApplyAsync (client, target, inputs, deadline.Token).ConfigureAwait (false);
			while (await GetUnconfiguredTarget (client, target, deadline.Token).ConfigureAwait (false) != null)
				await Task.Delay (250, deadline.Token).ConfigureAwait (false);
			return new (target.DeviceId, changed, true);
			}
		catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
			{
			throw new TimeoutException ("Driver configuration completion was not confirmed before the deadline. No command was retried.");
			}
		}
...
public sealed record DriverConfigurationResult (int DeviceId, bool Changed, bool Configured);