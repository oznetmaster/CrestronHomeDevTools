...
public sealed class ProcessorOperationLease : IProcessorOperationLease
	{
	...
	private ProcessorOperationLease (SftpClient sftp, string owner, string host, NetworkCredential credential, string fingerprint)
		{
		_sftp = sftp;
		_owner = owner;
		_host = host;
	...
	public static async Task<ProcessorOperationLease> AcquireAsync (string host, NetworkCredential credential, string fingerprint, string runId, CancellationToken token)
		{
	...
	public static Task<ProcessorOperationLease> AcquireAsync (string host, NetworkCredential credential, string fingerprint, string runId, TimeSpan wait, CancellationToken token)
	...
	public static async Task<ProcessorOperationLease> ResumeAsync (string host, NetworkCredential credential, string fingerprint, string owner, CancellationToken token)
		{
		if (!Guid.TryParseExact (owner, "N", out _)) throw new ArgumentException ("Invalid lease owner.");
		var sftp = new SftpClient (host, credential.UserName, credential.Password);
		var lease = new ProcessorOperationLease (sftp, owner, host, credential, fingerprint);
...
public interface IProcessorOperationLease : IDisposable
	{
	string Owner { get; }
