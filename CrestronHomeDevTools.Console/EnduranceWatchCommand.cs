...
internal static class EnduranceWatchCommand
	...
	internal static async Task<int> RunAsync (string[] args, TextReader input, TextWriter output, TextWriter error,
		...
		if (args.SequenceEqual (["--help"]))
			{
			await output.WriteLineAsync ("endurance-watch --worker FILE --observer FILE --notifications FILE --journal PRIVATE_DIRECTORY --send true\n" +
				"Perform one passive observation and notify the authorized destination, including observation failures.\n" +
				"Credentials arrive on stdin as {windows:{userName,password},smtp:{userName,password}}; omit windows for local observation.\n" +
		...
		try
			...
			for (int i = 0; i < args.Length; i += 2)
				if (i + 1 >= args.Length || args[i] is not ("--worker" or "--observer" or "--notifications" or "--journal" or "--send") ||
					!options.TryAdd (args[i], args[i + 1])) throw new ArgumentException ("Invalid watcher options.");
			if (options.Count != 5 || options["--send"] != "true") throw new ArgumentException ("Explicit notification authorization is required.");
			T Read<T> (string path)
				{
				using var file = File.OpenRead (path);
				if (file.Length > 65536) throw new ArgumentException ("Watcher input exceeds its size limit.");
			...
			if (observer.Task == null) throw new ArgumentException ("Missing task configuration.");
			...
			try
				...
				if (count == 0 || count == buffer.Length) throw new ArgumentException ("Supply bounded credentials on standard input.");
			...
			if (observer.Remote == null && credentials.Windows != null) throw new ArgumentException ("Local observation must not use remote credentials.");
			...
			await output.WriteLineAsync (JsonSerializer.Serialize (new { Health = report, Notification = notification }, JsonOptions));
		...
		Task WriteFailureAsync (string reason) => output.WriteLineAsync (JsonSerializer.Serialize (
			new { Health = report, Notification = (SubmissionEnduranceNotificationResult?)null, Failure = reason }, JsonOptions));
		}
	}