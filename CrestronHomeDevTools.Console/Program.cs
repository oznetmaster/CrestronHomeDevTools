...
if (args.Length == 0 && !Console.IsInputRedirected)
	...
	var configured = await RunAsync (["configure"]);
	if (configured != 0)
		return configured;
	...
	while (true)
		...
		try
			{
			await RunAsync (ConsoleCommandLine.Split (line), interactive: true);
			}
...
static async Task<int> RunAsync (string[] args, bool interactive = false)
	...
	if (args.Length == 0 || args[0] is "help" or "--help" or "-h")
		...
		Console.WriteLine ("""
            ...
            Options:
              ...
              --timeout seconds                   Operation wait timeout (default 120)
              --help                              Show this help
            ...
            Connection settings may instead be supplied through environment variables:
              ...
              CRESTRON_HOME_SSH_FINGERPRINT (required for deploy unless saved).

            Settings file fields: host, userName, password, certificateSha256,
              httpsPort (443), webSocketPort (49000), sshFingerprint (for deploy).
	...
	try
		...
		if (command == "reboot")
			...
			Console.WriteLine (JsonSerializer.Serialize (reboot, jsonOptions));
			Console.Error.WriteLine (reboot.Status switch
				{
					ProcessorRebootStatus.Cancelled => "Reboot cancelled; no command was sent.",
					ProcessorRebootStatus.Accepted => "Reboot request acknowledged. Processor startup has not been verified.",
