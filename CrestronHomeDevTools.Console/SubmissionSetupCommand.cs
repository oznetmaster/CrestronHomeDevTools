...
submission-setup check --run NAME [--store DIRECTORY]
submission-setup snapshot --run NAME --name SNAPSHOT [--store DIRECTORY]
...
""");
  ...
  try
  {
   ...
   for (int i = 1; i < args.Length; i += 2)
    if (i + 1 >= args.Length || args[i] is not ("--store" or "--kind" or "--run" or "--name" or "--snapshot") || !options.TryAdd (args[i], args[i+1])) throw new ArgumentException ();
   ...
   switch (args[0])
   {
    ...
    case "check":
     var errors = store.CheckSubmissionSetup (Need ("--run"));
    ...
    case "snapshot":
     var saved = store.CreateSubmissionSetupSnapshot (Need ("--run"), Need ("--name"));
     output.WriteLine (JsonSerializer.Serialize (new { saved.Name, saved.Revision, saved.SavedUtc, IsAuthorization = false })); return 0;
    case "prepare":
