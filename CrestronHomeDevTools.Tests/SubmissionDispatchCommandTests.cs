...
public sealed class SubmissionDispatchCommandTests
	...
	public void SetUp ()
		...
		File.WriteAllText (package, "synthetic package"); File.WriteAllText (form, "synthetic form");
	...
	public async Task PinnedSettingsAndStdinCredentialsReachDispatchWithoutBeingPrinted ()
		{
		using var output = new StringWriter (); using var error = new StringWriter (); int calls = 0;
	...
	public async Task InvalidSettingsNeverDispatch (string failure)
		{
		string[] args = Args ();
		if (failure == "hash") args[3] = new ('0', 64);
		else if (failure == "missing-flag") args = args[..^1];
		else
			{
			if (failure == "unknown-setting") File.WriteAllText (_path, File.ReadAllText (_path).Insert (1, "\"unreviewed\":true,"));
			if (failure == "duplicate-setting") File.WriteAllText (_path, File.ReadAllText (_path).Insert (1, "\"schemaVersion\":1,"));
			if (failure == "missing-setting") File.WriteAllText (_path, File.ReadAllText (_path).Replace ("\"schemaVersion\":1,", ""));
			if (failure == "relative-package") Save (_settings with { PackagePath = "test.pkg" });
			if (failure == "nested-receipts") Save (_settings with { MailReceiptDirectory = Directory.CreateDirectory (Path.Combine (_settings.JournalDirectory, "mail")).FullName });
			if (failure == "unencrypted-port") Save (_settings with { SmtpPort = 25 });
			if (failure == "oversized-settings") File.WriteAllText (_path, new string (' ', 1024 * 1024 + 1));
			args = Args ();
			}
		int calls = 0; using var output = new StringWriter (); using var error = new StringWriter ();
	...
	public async Task InvalidCredentialsNeverDispatch (string input)
		{
		if (input == "oversized") input = new string ('x', 32769);
		if (input == "duplicate") input = Credentials.Insert (1, "\"smtpPassword\":\"other-password\",");
		int calls = 0;
	...
	public async Task DispatchRevalidatesBothStepsAndCompletedReplayDoesNothing ()
		{
		var steps = new List<SubmissionDeliveryStep> (); var transport = new Transport ();
	...
	private sealed class Transport : ISubmissionDeliveryTransport
		...
		public Task<SubmissionUploadReceipt> UploadAsync (Stream package, string filename, CancellationToken token)
			{ Uploads++; return Task.FromResult (new SubmissionUploadReceipt ("https://example.test/package", "synthetic-upload")); }
