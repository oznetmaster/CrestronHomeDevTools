...
| S03 | Candidate/evidence manifest and strict gate | Reject stale/cross-package evidence, skipped controls, incomplete restoration, missing files and duplicate IDs; no hard-coded total test counts | Evidence evaluator tested; authenticated producer/bundle integration and subcondition policies pending |
| S04 | Android automation library and NUnit fixture foundation | Target verification, stable selectors, bounded waits, screenshots/hierarchy, cancellation and serial selection; repeated read-only runs pass | Script proof passed minimized; .NET primitives/offline tests added; hardware/fixture integration pending |
