...
| S10 | Crestron upload and email adapters | Confirm service behavior, capture download URL and mail receipt; uncertain outcomes require reconciliation; crash-safe duplicate prevention | Delivery journal/transport boundary and reconciliation covered by synthetic tests; version 1.8.0 also rechecks trusted authorization before each external step, with a pinned offline revalidation process bridge tested through simulated dispatch, preserving confirmed upload state when email is refused. [Controlled uploader validation](submission/UploaderInspection.md) passed one authorized small-file upload and byte-identical download; the 1.8.0 [upload provider](submission/CrestronUploader.md) passes offline tests and actual-response replay. The live adapter encountered an already-uploaded response, and GET-only reconciliation verified the original bytes while preserving its first failed attempt. The 1.8.0 [SMTP provider](submission/SmtpDelivery.md) composes the approved message and records server acceptance, with offline interruption/retry tests. A private plaintext self-mail check verified TLS/login, sender permission, server acceptance and inbox receipt. The source [protected delivery command](submission/DeliveryCommand.md) now composes both providers with fresh revalidation and the journal. Fresh-file creation through the upload adapter, the signed-PDF mail path, protected CI rehearsal, broader recovery and final delivery remain pending |
...
The source [Android evidence audit](submission/AndroidEvidence.md) checks retained discovery/TRX coverage, candidate/run identity, fixture and capture digests, capture intervals and matching restoration. It requires independently trusted pins and rejects Debug candidates. A complete read-only suite against an unchanged Release candidate passed the released Android stage and auditor, including cleanup. That private integration run does not prove the complete public workflow or protected CI handoff. Source work after 1.8.0 connects this audit to [review preparation](submission/ReviewStage.md), retains its records privately through signing and delivery checks, and rejects changed or missing Android evidence. Producer authentication and complete requirement mapping remain necessary; the audit creates no passing official observations.

## Hardware and failure handling

The DevTools [endurance collector and scheduled CLI](submission/EnduranceCollection.md), released in 1.6.0, retain candidate-bound functional samples across invocations and stop on gaps, changed environments and interrupted probes. Real processor infrastructure checks and short consumer-owned functional observations have passed, including deliberate driver-restart rejection. DevTools 1.7.0 adds public plan validation and bounded processor uptime observations. Version 1.8.0 includes [Windows scheduler scripts](submission/WindowsEnduranceWorker.md) in the console ZIP, with offline interruption checks. Real credential provisioning, independent alerts, OS restart validation and the final candidate endurance period remain incomplete.
