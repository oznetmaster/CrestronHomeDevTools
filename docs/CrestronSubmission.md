...
| S04 | Android automation library and NUnit fixture foundation | Target verification, stable selectors, bounded waits, screenshots/hierarchy, cancellation and serial selection; repeated read-only runs pass | Foundation passed in a Wiser development workflow. Optional name binding is integrated in the Wiser fixture using released DevTools 1.5.0; both cases passed against an existing Debug gateway in the minimized Google emulator, with name/Home restoration and unchanged inventory/state. Exact Release-candidate validation remains pending |
