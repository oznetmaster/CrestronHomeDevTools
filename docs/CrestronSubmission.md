...
| S04 | Android automation library and NUnit fixture foundation | Target verification, stable selectors, bounded waits, screenshots/hierarchy, cancellation and serial selection; repeated read-only runs pass | .NET saved-endpoint inspection and Home restoration passed twice on minimized Android under shared reservations; scoped fields/overlay guards tested offline; full NUnit candidate run, active-route and installed-instance binding pending |
