...
| S10 | Crestron upload and email adapters | Confirm service behavior, capture download URL and mail receipt; uncertain outcomes require reconciliation; crash-safe duplicate prevention | Delivery journal/transport boundary and reconciliation covered by synthetic tests. Authenticated uploader inspection, real adapters, provider recovery and live delivery remain pending |
| S11 | Optional final driver-submission CI stage and Wiser pilot | Explicit driver-release opt-in and trusted artifact; client/library and processor-test releases never enter it. Dry-run bundle first, then one authorized real submission with durable receipt; rerun does not submit twice. Disabled/unconfigured submission leaves ordinary CI and publication usable | Private unsigned review preparation passes synthetic integration, including a successful Windows service-account workflow. Reusable template prepared; real driver evidence/template pilot, signing and delivery remain pending |
...
The source [coverage blueprint generator](submission/CoveragePlanning.md) now produces draft policies, complete form mappings and scoped execution contracts. Wiser's blueprint covers every Extension item and declared UI target, including configuration, conditional schedules, two-instance repetitions and quantitative outage/endurance obligations. It remains a draft: all producers are unbound, real Release evidence is missing, and the interpretation of two instances for a platform driver versus its managed children must be confirmed. Testing isolation between two child rooms is useful but is not automatically substituted for that requirement.
...
The Android proofs cover navigation, displayed weather sections and repeated .NET inspection of the selected system's saved local endpoint with Home restoration. They do not establish the correctness of all Home/Room icons, control timing, physical feedback, Configure/Setup dialogs or submission readiness. The app's saved connection details are not independent proof of its active route or a particular installed driver instance. The separate [name-challenge pilot](DriverUiBinding.md) observed a freshly renamed Wiser gateway in the app and restored it, providing stronger instance association. It does not authenticate the app route or the candidate artifact.

The subsequent [Wiser fixture integration](https://github.com/oznetmaster/WiserHeatCrestronDriver/blob/master/WiserHeatCrestronDriver.AndroidTests/README.md) uses DevTools 1.5.0 and TestAdapter 1.7.1. With private `AllowNameBinding` enabled, both cases passed against the already-installed Debug gateway; fourteen screenshot/hierarchy pairs matched their retained hashes. Original names, inventory, observed gateway states and Home were restored, and both reservations were released. No physical control command was sent. This integration check did not deploy a new package or repeat the full gated workflow, and is not Release-candidate acceptance evidence.

The full workflow was then repeated with that option and the published 1.7.1 CLI. It passed 96 desktop tests, 53 processor tests, three live hub reads, the gated Debug update, installed readiness and both UI cases. It removed its temporary instance and archive, preserved all original installed-device identities and checked room/gateway control states, verified fourteen capture pairs and released both reservations. This establishes the combined Debug workflow with the new binding, while exact Release-candidate acceptance remains open. A separate installed-runner probe confirmed ADB connectivity from NETWORK SERVICE to the existing Google emulator/app; it sent no UI input and did not validate full service-driven UI execution.
