...
The source [Android evidence audit](submission/AndroidEvidence.md) checks retained discovery/TRX coverage, candidate/run identity, fixture and capture digests, capture intervals and matching restoration. It requires independently trusted pins and rejects Debug candidates. A complete read-only suite against an unchanged Release candidate passed the released Android stage and auditor, including cleanup. That private integration run does not prove the complete public workflow or protected CI handoff. Version 1.9.0 connects this audit to [review preparation](submission/ReviewStage.md), retains its records privately through signing and delivery checks, and rejects changed or missing Android evidence. Producer authentication and complete requirement mapping remain necessary; the audit creates no passing official observations.
