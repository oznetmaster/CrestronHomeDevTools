...
`State: "Ready"` gives CLI exit 0 after both online and ready were observed for an ordinary managed child. `State: "ConfigurationRequired"` gives exit 3, with the pending wizard step in the private journal; the coordinator never guesses or applies its values. Failures/timeouts retain their journal. Uncertain console mutations also retain the processor lease for reviewed recovery. Never interpret a null configuration step alone as readiness.

The current source also recognizes a child that immediately appears as an unlocated wrapper with one native light load. It verifies the platform version, wrapper identity and advertised managed-device ID, and the load's identity and requested room. A loaded, online wrapper and an available dimmer command with a valid level establish current readiness. `DeviceId` remains the commissioned wrapper ID; `NativeLoadId` identifies the actual light. Offline loads may temporarily omit dimmer commands and values, so they remain not ready rather than being accepted or mistaken for another device. No configuration wizard is invented for a wrapper that exposes none.

After interruption, `ManagedDeviceCommissioning.ObserveCreatedAsync(client, commissioningJournal, cancellationToken)` reads the original request, successful commission response and created-child receipt, checks their agreement and observes the existing child. It sends no commands and does not rewrite the journal or historical outcome. Save its result separately. A missing final result is not permission to commission again. A current `Ready` observation does not prove that configuration survives reload; persistent setup must be checked separately before unattended testing. These source APIs are not in the previously released 1.19.0 bundle.
...
Use the same processor profile as commissioning. This command acquires its own shared lease; do not invoke it inside a workflow already holding that lease. The private journal is not a cross-processor device identifier. Its commissioning result must be terminal (`Ready` or `ConfigurationRequired`), and the recorded identity must still match. This operation removes the child assignment; it does not replace a package or reboot/unload the platform.

The current source also handles a managed light that becomes an unlocated wrapper with one native load after activation. The wrapper must retain the receipt's ID, parent, name and model and advertise exactly the original managed-device ID. Its one native load must have the expected name, model and room, advertise `lightType:variant=load` and the location-removal command, and have no descendants. Cleanup sends the command to that load and requires **both** load and wrapper to disappear, with every unrelated device preserved. Other layouts are rejected. This source addition has offline regression coverage and a successful CP4-R native-light cleanup check: both owned devices disappeared and unrelated inventory was preserved. That check does not establish the full unattended workflow or replacement-light readiness. It is not included in the previously released 1.19.0 bundle.

Choose the intended persistent light name in the commissioning request. In the observed native-light layout, changing only the generated load's name was undone by a later driver reload when its wrapper restored the commissioning name. Do not treat an immediate rename as proof of persistent configuration.
...
If creation returned a confirmed child ID but readiness validation failed before `result.json` was written, ordinary cleanup still refuses automatic removal. After inspecting that original attempt, `RemoveCreatedAfterInspectionAsync` provides explicit recovery using the same arguments and ownership, platform, topology and preservation checks. The caller must hold the processor lease and verify physical restoration first. A missing successful commission response, contradictory receipt or prior cleanup attempt remains an error. Recovery records that the original readiness result was absent; it never creates a passing commissioning result. This source API is not yet included in the 1.19.0 bundle.
