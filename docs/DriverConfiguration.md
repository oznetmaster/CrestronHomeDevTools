...
MC4-R / Home 4.11.322 hardware validation read the installed Wiser configuration, preserved it when configure was invoked again, installed a separate temporary Wiser instance and completed its Connection and HeatSettings wizard. The temporary instance was removed afterward, and the original instance was verified ready with unchanged settings. The ordinary removal helper conservatively refused the shared reload scope; that known scope was inspected and handled explicitly during cleanup. No heating controls were operated.
