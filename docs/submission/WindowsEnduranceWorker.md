...
Released versions through 1.18.1 retain each invocation's intent, child process identities, private stdout/stderr and completed result under `scheduler-state/attempts`. Current source removes successful attempts' temporary diagnostics after atomically saving `scheduler-state/status.json` and a compact `history.jsonl` entry. The history rotates at 1 MiB, retaining one previous file. The current status still contains the complete checkpoint required by health validation; that growing checkpoint is no longer retained again for every successful wake-up. Actual collector samples are unchanged.

Failed and interrupted attempts keep their full original diagnostics. Successful diagnostic cleanup uses a separate completed-attempt name so an interrupted deletion cannot be mistaken for an unfinished collector call. It resumes cleanup on the next invocation, checks that the retained result succeeded, refuses linked or unexpected content, and never repeats a collector operation to perform cleanup. These are operational records, not submission evidence; only the collector's revalidated export can enter the evidence pipeline. Do not replace pinned scripts during an existing collection; the reduced diagnostic policy applies to newly configured runs using the updated scripts.
...
The updated exporter allows 100,000 filesystem entries per inventory by default; `-MaximumEntries` can select a bound from 1 to 1,000,000. The earlier 20,000-entry bound could reject an ordinary completed 24-hour run because scheduler diagnostics are recorded more often than functional samples. The chosen bound is retained in the intent and completion receipts. Existing 64 MiB per-file and 1 GiB per-inventory byte limits, hash verification and refusal to overwrite a partial export still apply. Increasing this retention bound does not change sample criteria or discard diagnostics. Use a fresh destination after inspecting any previous limit failure.
