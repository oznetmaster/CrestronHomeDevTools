...
The API itself does not install a Windows service or scheduled task. Scheduler scripts are included in the console archive starting in 1.8.0 and described in the [Windows worker guide](WindowsEnduranceWorker.md). The CLI described below supports separate scheduler invocations. Its private files and credentials must survive a worker restart. Protect shared physical devices separately when other processors can access them.
