...
**Source preview, not a completed end-to-end validation.** This branch implements release discovery/intake, persistent sequencing and a [Windows automation worker](AutomationWorker.md) connecting public tests, endurance, review, signing, delivery and retention. Real hardware validated Windows/processor tests and cleanup. An actual Windows restart validated background startup; synthetic document integration validated exact-authority waits, signing, provider sequencing and duplicate-send prevention. Live GitHub discovery/intake registered a published release without advancing its tests. The configured real Android/endurance route and final fresh release-to-delivery demonstration remain pending. The existing [manual workflow](WorkflowSetup.md) remains the supported GitHub stage route. Use the [single starting document](START-DRIVER-SUBMISSION.md) for an assistant-led submission.
...
| Work | Done when |
|---|---|
| Stage validation | Exercise configured Android and endurance handoffs on real hardware as part of the full route. Review/signing/delivery/retention adapters have synthetic integration coverage, and Windows/processor tests have real hardware evidence. |
...
| Durable continuation | The startup task has passed an actual reboot under LocalService with an empty registry. Validate real credential access and waiting-stage resumption under the configured identities. Shared resources still use the existing leases. |
