...
| File | Purpose |
| --- | --- |
| `policy.json` | One evidence requirement per scoped assertion, compatible with `SubmissionValidation`. Minimum durations, target/method, required outcome, response deadline and restoration obligations are explicit; no fixed passing-test total is used. |
...
Endurance execution requires a reviewed `maximumSampleGapSeconds`. The generator leaves it null rather than inventing a cadence; a policy with an endurance method and no cadence cannot pass. Once approved, samples must cover both ends of the observation interval, increase strictly, stay within the maximum gap, pass, and reference retained files. This catches missing observation intervals; it does not establish producer authenticity, measurement accuracy or prove the specific functional action performed between samples. Outage instrumentation and quantified event/response accuracy still require approved producers.
...
When an authenticated producer used a narrower policy, the source [reviewed evidence mapping API and CLI](EvidenceMapping.md) can carry explicitly equivalent observations into the destination policy while retaining the originals. It does not resolve draft producer bindings or replace the behavioral coverage review.
