...
**Availability:** this command is implemented in source after DevTools 1.9.0. It is not present in the 1.9.0 download. Build the complete console with [the documented build command](ConsoleTools.md#building-the-console-from-source) until a release containing it is available.
...
6. Use [the final-stage workflow template](submission-delivery.yml.example), setting its reviewed tooling inventory, `DELIVERY_SETTINGS` and `DELIVERY_SETTINGS_SHA256`. Supply upload and SMTP credentials through protected standard input as described in [Delivery command](DeliveryCommand.md). Never put credentials in this setup JSON, command arguments or build logs. Environment approvals, runner isolation and ACLs must be configured by the operator; this preparation command does not provision them.
