...
The production watcher's combined discovery/dispatch cycle also has integration coverage: an initially missing release asset becomes available, intake registers the frozen candidate, and the same cycle starts its workflow. A later cycle with a newly created adapter resumes the original waiting operation. Duplicate discovery neither starts another run nor restarts failed or review-ready work. These checks use simulated GitHub responses, source transport and domain operations with real intake, registry, receipt hashes and workflow persistence; they do not establish a real GitHub publication or Windows reboot.
