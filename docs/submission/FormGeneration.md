...
An item is checked when every applicable mapped assertion is supported. Policy-permitted `NotApplicable` subconditions are separately explained; an entirely non-applicable item remains unchecked and labelled N/A. Failed, partial, inconclusive, missing or duplicate observations cannot yield a complete evidence-backed form; use the explicit declared-gaps path when appropriate.
