...
The template itself has not run on the installed GitHub service. Its underlying command is covered by an actual .NET/Python integration test using clearly labelled synthetic evidence, including rejection when evidence changes between form generation and bundle creation. See the [full submission plan](../CrestronSubmission.md) for remaining hardware and delivery work.
