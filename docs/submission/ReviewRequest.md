...
The source [review delivery API](ReviewDelivery.md) can preserve this disposition and exact correspondence at the provider boundary. Connecting this prepared receipt to final pinned authorization, per-step revalidation and the protected delivery command is still in progress. Do not invoke the legacy complete-only delivery command with these files or relabel an unsigned attachment as a signed form.
