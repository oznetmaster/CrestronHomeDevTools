...
The current-source [upload provider](CrestronUploader.md) passes offline failure-path tests and replay of the retained real responses. A subsequent live provider test observed an already-uploaded response with one download link; read-only reconciliation verified the exact bytes without repeating the POST. This does not establish whether the service deduplicates by name, content or another criterion. Fresh-file creation through that provider, email-provider configuration and a controlled email test remain unfinished. An actual submission email must follow Crestron's published procedure, including the download link and signed self-test PDF. Private evidence archives and signature source images must not be attached.
