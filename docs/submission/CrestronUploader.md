...
The earlier live transport probe used a separate inspection helper. The new provider itself has not made a live upload. Production use still requires a controlled live adapter run, a configured and validated mail provider, current authorization and the final driver evidence/form. Ordinary releases remain independent of this optional submission stage.
