...
The [release automation controller](ReleaseAutomation.md) is a source preview. Its separate [Windows worker](AutomationWorker.md) has exercised intake, Windows/processor tests and cleanup on real hardware; the entire submission route remains incomplete. The intake command alone does not start tests. Do not substitute a partial rehearsal or synthetic stage tests for a real submission. The [setup form](SetupApp.md) can reduce repeated questions, but check availability in the selected published version before prescribing it to another developer.
