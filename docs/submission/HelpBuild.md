...
When changing `AssemblyName`, also check how the driver's entry point loads its embedded JSON definition. In the Crestron SDK 27.0.24, the default `DriverImplementationResources.FromCreationArgs(args, typeof(EntryPoint))` lookup expects an embedded resource ending in the assembly name plus `.json`. Renaming the DLL while retaining the previous JSON resource name can produce a package that imports successfully but fails during driver initialization. Either align the embedded resource name with the new assembly name or use the overload that explicitly supplies its full resource name. Verify the lookup against the final merged DLL before deployment; matching DLL/DAT/PDF filenames alone does not verify runtime resource loading.
