...
Version 1.13.0 and later accept the output of [unsigned request preparation](ReviewRequest.md): `UnsignedSelfTest` or `DisclosureOnly`. Version 1.14.0 and later also accept the declared-gap output of [signed review preparation](SigningStage.md). It never applies signatures during delivery or bypasses required-document validation.
