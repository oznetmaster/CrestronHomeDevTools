...
4. Run the approved test producer against that candidate. Preserve observations and their referenced files in a private evidence store.
   For Android workflow output, run the bundled [Android evidence audit](AndroidEvidence.md) against independently retained candidate/run/fixture/discovery pins before preparing scoped observations. Its successful report does not supply an official requirement outcome.
...
The CLI hashes and parses the same candidate/policy/observation bytes, and holds those files and the package/form open during checking. It hashes each retained evidence file while reading it. This is validation at a point in time, not an immutable archive or an authenticated test producer. The [private evidence bundle commands](EvidenceBundle.md) now snapshot the referenced inputs, validate the archive's own copies and allow later checks against independently retained bundle/candidate digests. Signing must consume that protected snapshot and revalidate its digests. A passing report alone does not authorize submission or prove that the policy measured everything required.

No fixed test count is used. Each policy requirement needs exactly one observation. Use a distinct stable ID for each required subcondition; a form checkbox may depend on several IDs. The [form generator](FormGeneration.md) requires all of them. The CLI currently validates the supplied policy, not its completeness against the official form. Each driver still needs an approved complete policy and reviewed mapping.
...
Website-only support is available in the released DevTools 1.10.0 console; use the complete archive described in [Console tools](ConsoleTools.md). The existing `publicSupportEmail` field remains supported. Specify at least one approved contact; if both are configured, both must match the package metadata. These are public support contacts, separate from the private sender used for submission correspondence.
...
An ordinary GitHub/NuGet release may use the existing hardware-unavailable override. It must leave Crestron submission pending; it cannot fabricate observations or bypass this submission check. The retained bundle, unsigned review, authorized signing and delivery-preparation stages are implemented and tested with synthetic inputs. Upload and email adapters are available in the protected delivery workflow; real candidate review, exact-form signature authorization and protected-console submission have now been exercised. Each developer still supplies and validates their own environment and evidence; see [verified scope](ValidationStatus.md). See the [complete implementation plan](../CrestronSubmission.md).
