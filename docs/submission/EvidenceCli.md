...
4. Run the approved test producer against that candidate. Preserve observations and their referenced files in a private evidence store.
   For Android workflow output, run the source [Android evidence audit](AndroidEvidence.md) against independently retained candidate/run/fixture/discovery pins before preparing scoped observations. Its successful report does not supply an official requirement outcome.
...
An ordinary GitHub/NuGet release may use the existing hardware-unavailable override. It must leave Crestron submission pending; it cannot fabricate observations or bypass this submission check. The retained bundle, unsigned review, authorized signing and delivery-preparation stages are implemented and tested with synthetic inputs. Real candidate review, signature authorization, supported upload/email adapters and end-to-end delivery remain pending. See the [complete implementation plan](../CrestronSubmission.md).
