...
A later complete Wiser development workflow also passed with name binding enabled, using published NUnit CLI 1.7.1 and DevTools 1.5.0 in the Android fixture. It ran desktop, processor and live hub tests, updated the Debug driver, checked installed readiness, executed both UI cases and removed the temporary test instance/archive. All original installed-device identities and checked room/gateway control states were preserved; fourteen capture pairs and both released reservations were verified. The Google emulator remained minimized. The cached Home catalogue entry can remain until a planned reboot even after archive storage is removed.
